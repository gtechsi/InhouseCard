...
import { collection, query, orderBy, getDocs, where, doc as firestoreDoc, getDoc, updateDoc, Timestamp } from 'firebase/firestore';
...
export default function Orders() {
  ...
  async function fetchOrders() {
    try {
      ...
      // Base query
      let baseQuery = query(ordersRef, orderBy('created_at', 'desc'));

      // Add filters
      if (statusFilter !== 'all') {
        baseQuery = query(ordersRef, where('status', '==', statusFilter), orderBy('created_at', 'desc'));
      }
      ...
      const ordersData = await Promise.all(
        querySnapshot.docs.map(async (orderDoc) => {
          ...
          const profileRef = firestoreDoc(db, 'profiles', orderData.user_id);
